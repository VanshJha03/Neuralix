...
export async function POST(req: NextRequest) {
  ...
  try {
    const response = await ai.models.generateContent({ model: 'gemma-4-31b-it', contents: `Find 5 viral trends last 4-7 days in: ${activeLabels}. Format: PLATFORM | TOPIC | VIRAL_HOOK`, config: { tools: [{ googleSearch: {} }] } });

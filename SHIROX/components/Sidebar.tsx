...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const Item = ({ id, icon: Icon, label }: { id: ViewType, icon: any, label: string }) => (
    ...
    >
      ...
      {!isCollapsed && <span className="text-[10px] font-black uppercase tracking-[0.2em] truncate">{label}</span>}

      {isCollapsed && (
        <div className="absolute left-full ml-4 px-3 py-1 bg-zinc-900/90 border border-zinc-800 text-white text-[10px] font-black uppercase tracking-widest rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-all duration-300 z-50 whitespace-nowrap shadow-2xl backdrop-blur-xl">
          {label}
        </div>
      )}
    </button>
  ...
  return (
    <div className={`h-full bg-zinc-900/30 backdrop-blur-3xl border-r border-zinc-800/50 flex flex-col transition-all duration-500 ${isCollapsed ? 'w-20' : 'w-72'} z-30`}>
      {/* Brand */}

...
const TierCard: React.FC<TierCardProps> = ({ name, price, period, features, icon, popular, onSelect, accent, isFree, loading }) => (
    <div className={`relative group p-6 rounded-[2.5rem] bg-zinc-950 border ${popular ? 'border-white/20' : 'border-zinc-900'} hover:border-white/30 transition-all duration-500 flex flex-col h-full shadow-2xl`}>
        {popular && (
            <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1 bg-white text-black text-[10px] font-normal uppercase tracking-widest rounded-full shadow-xl">
                Most Popular
            </div>
        )}
        ...
        <h3 className="text-xl font-normal text-white tracking-tighter mb-1 uppercase lg:text-2xl">{name}</h3>
        <div className="flex items-baseline gap-1 mb-8">
            <span className="text-3xl font-normal text-white font-normal">{price}</span>
            {period && <span className="text-xs text-zinc-600 font-normal uppercase tracking-widest">{period}</span>}
        </div>
        ...
        <button
            onClick={onSelect}
            disabled={loading}
            className={`w-full py-4 rounded-xl font-normal uppercase tracking-[0.2em] text-[10px] transition-all transform active:scale-95 flex items-center justify-center gap-2 ${popular ? 'bg-white text-black hover:bg-zinc-200' : isFree ? 'bg-transparent border border-zinc-800 text-zinc-600 hover:border-zinc-600 hover:text-white' : 'bg-zinc-900 border border-zinc-800 text-zinc-400 hover:border-white/40 hover:text-white'}`}
        >
        ...
        <div className="fixed inset-0 z-[100] min-h-screen bg-black p-6 lg:p-12 overflow-y-auto">
            <div className="max-w-6xl mx-auto flex flex-col items-center">
                <div className="mb-12 flex flex-col items-center text-center">
                    <NeuralLogo size={60} />
                    <h2 className="text-4xl lg:text-6xl font-normal text-white mt-8 tracking-tighter font-normal" style={{ fontFamily: "'Orbitron', sans-serif" }}>Choose Your Evolution</h2>
                    <p className="text-zinc-600 text-[10px] lg:text-[12px] uppercase tracking-[0.5em] font-normal mt-4 max-w-md">Upgrade your neural capacity. Select the tier that matches your trajectory.</p>
                </div>
                ...
                <p className="text-zinc-800 text-[9px] uppercase tracking-widest font-normal text-center">
